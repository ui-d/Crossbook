...
### 1.4 Register the live Stripe webhook
...
### 1.6 Verify your domain in Resend
...
### 1.7 Smoke-test the live deploy
...
## Section 2 — Rotate the leaked secret
...
- **DPA generator API key** (Day 17 finishing touch): pick Iubenda, Termly, or Vanta DPA generator, get an API key, drop into `DPA_GENERATOR_API_KEY` in Vercel. Once set, I can wire `/dpa` to render a per-customer countersigned PDF instead of the current static template.
